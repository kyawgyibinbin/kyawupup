...
@Kyawwa.on_message(filters.private & filters.command('total'))
async def sts(c, m):
    if m.from_user.id != Config.OWNER_ID:
        return 
    total_users = await kyawwa.total_users_count()
    await m.reply_text(text=f"Total user(s) {total_users}", quote=True)


@Kyawwa.on_message(filters.private & filters.command("search"))
